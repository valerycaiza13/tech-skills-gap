...
from skills_gap_analysis import (
    load_data,
    calcular_gaps,
    resumen_por_rol,
    skills_mas_criticas,
    recomendaciones_formacion
)
...
# -------------------------
st.set_page_config(
    page_title="Tech Skills Gap Dashboard",
    layout="wide"
)
...
try:
    ...
    st.error("❌ Error al cargar los datos o ejecutar el análisis.")
    st.exception(e)
    st.stop()
...
rol_sel = st.sidebar.multiselect(
    "Rol",
    options=roles,
    default=roles
)
...
skill_sel = st.sidebar.multiselect(
    "Skills",
    options=skills,
    default=skills[: min(10, len(skills))]
)
...
col1.metric(
    "👥 Empleados",
    int(df_f["employee_id"].nunique())
)

col2.metric(
    "🧠 Skills",
    int(df_f["skill_name"].nunique())
)

col3.metric(
    "⚠️ Gap severity total",
    round(df_f["gap_severity"].sum(), 2)
)

st.divider()

# -------------------------
# Resumen y skills críticas
# -------------------------
left, right = st.columns([1, 1])

with left:
    st.subheader("📌 Resumen de gaps por rol")
    st.dataframe(
        resumen[resumen["rol_actual"].isin(rol_sel)],
...
if not chart_df.empty:
    chart_df_plot = chart_df.set_index("skill_name")[["gap_severity"]]
...
# -------------------------
st.subheader("🧑‍💻 Detalle de gaps por empleado y skill")

st.dataframe(
    df_f.sort_values(
        ["rol_actual", "employee_id", "gap_severity"],
...
recs_f = recs[
    recs["rol_actual"].isin(rol_sel)
    & recs["skill_name"].isin(skill_sel)
].copy()
